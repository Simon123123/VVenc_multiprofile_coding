...
def main():
    ...
    for f in list_files_trace:
        ...
        data = np.load(f).astype(np.int8)
        
        shape_w = data[..., 0]
        
        shape_h = data[..., 1]
        
        size_map = 32 * 4  // args.cell_size 
        
        ops_scale = args.cell_size // 4
        
        if args.metric in ['max_size_map_1d', 'max_size_map_2d']:
            shape_w = shape_w.reshape((-1, size_map, ops_scale, size_map, ops_scale)).max(axis=(2, 4))
            shape_h = shape_h.reshape((-1, size_map, ops_scale, size_map, ops_scale)).max(axis=(2, 4))
        else:
            shape_w = shape_w.reshape((-1, size_map, ops_scale, size_map, ops_scale)).min(axis=(2, 4))
            shape_h = shape_h.reshape((-1, size_map, ops_scale, size_map, ops_scale)).min(axis=(2, 4))        
